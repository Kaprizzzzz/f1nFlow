...
export class CategoriesService {
  private readonly incomeCategoriesSubject = new BehaviorSubject<
    CategoryItem[]
  >([]);
  private readonly expenseCategoriesSubject = new BehaviorSubject<
    CategoryItem[]
  >([]);
  ...
  addCategory(
    ...
    if (
      list.some((item) => item.name.toLowerCase() === normalized.toLowerCase())
    ) {
      return false;
    }
  ...
  renameCategory(
    ...
    const nextList = list.map((item) =>
      item.name.toLowerCase() === oldLower
  ...
  deleteCategory(type: 'plus' | 'minus', categoryName: string): boolean {
    ...
    const nextList = list.filter(
      (item) => item.name.toLowerCase() !== targetLower,
    );
